...
def _load_model(checkpoint_path, *, device = "cuda", precision = torch.bfloat16, use_tp = False):
    ...
    with torch.device('meta'):
        model = Transformer.from_name(str(path.parent/"config.json"))

    checkpoint = torch.load(str(checkpoint_path), mmap=True, weights_only=True)
    model.load_state_dict(checkpoint, assign=True)

    if use_tp:
        from model.mixtral_tp import apply_tp
...
transformer_configs = {
    "mixtral": dict(block_size=32768, n_layer=32, n_head=32, n_local_heads=8, dim=4096, intermediate_size=14336, rope_base=1000000.0, num_experts=8, num_activated_experts=2),
}
